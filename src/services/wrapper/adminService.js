...
export const adminUpdateCategory = async (categoryId, data) => {
  const res = await patchData(ENDPOINTS.admin.categories.update(categoryId), data) // api.patch(`/admin/categories/${categoryId}`, data);
  return res.data;
};
...
export const assignReportToEmployee = async (reportId, employeeId) => {
  const res = await patchData(ENDPOINTS.admin.reports.assign(reportId), { employee_id: employeeId }) // api.patch(`/admin/reports/${reportId}/assign`, { employee_id: employeeId });
  return res.data;
};
...
export const adminApproveAreaSuggestion = async (areaId) => {
  const res = await patchWithoutData(ENDPOINTS.admin.areaSuggestions.approve(areaId)) // api.patch(`/admin/area-suggestions/${areaId}/approve`);
  return res.data;
};

// PATCH /admin/area-suggestions/{areaId}/reject
export const adminRejectAreaSuggestion = async (areaId) => {
  const res = await patchWithoutData(ENDPOINTS.admin.areaSuggestions.reject(areaId)) // api.patch(`/admin/area-suggestions/${areaId}/reject`);
  return res.data;
};